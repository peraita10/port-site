...
import{clamp01,normalizedProgress,selectStage,coverTransform,processScrollOffset,appendTrailPoint,cursorToneFromRgb}from'../src/interaction/math.js';
...
test('cursorToneFromRgb chooses a contrasting cursor tone',()=>{assert.equal(cursorToneFromRgb(242,240,234),'dark');assert.equal(cursorToneFromRgb(255,255,255),'dark');assert.equal(cursorToneFromRgb(17,17,15),'light');assert.equal(cursorToneFromRgb(100,54,239),'light')});
