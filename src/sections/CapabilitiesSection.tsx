...
export default function CapabilitiesSection(){
 ...
 return <section id="capabilities" className={`capabilities ${current.className}`}><div className="cap-stage" style={{'--px':pointer.x,'--py':pointer.y} as React.CSSProperties}><span className="cap-stage-label">02 / CAPABILITIES</span><div className="cap-visual" aria-hidden="true"><div className="cap-scene-grid"/><div className="cap-scene-frame"><span>PORT.SITE</span><b>02</b></div><div className="cap-scene-word">MAKE<br/><em>IT</em><br/>VISIBLE</div><div className="cap-scene-orbit"><i/><i/></div><div className="cap-scene-code">&lt;main&gt;<br/>grid / type / motion<br/>&lt;/main&gt;</div><div className="cap-scene-live"><span>LIVE</span><i/></div></div><p>{current.label}</p></div><div className="cap-list">{capabilities.map((item,index)=><button key={item.name} onMouseEnter={()=>setActive(index)} onFocus={()=>setActive(index)} onClick={()=>setActive(index)} data-active={index===active} aria-pressed={index===active}><span>0{index+1}</span><strong>{item.name}</strong><i>↗</i></button>)}</div></section>;
}
